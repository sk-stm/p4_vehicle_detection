...
def convert_color(img, conv='RGB2YCrCb'):
    if conv == 'RGB2YCrCb':
        ...
        raise NotImplementedError("Color conversion " + str(conv) + " not implemented!")
...
def extract_features(imgs, color_conv=None, spatial_size=(32, 32),
                        hist_bins=32, orient=9, 
                        pix_per_cell=8, cells_per_block=2, hog_channel=0,
    ...
    for file in imgs:
        ...
        assert image.dtype == np.float32 and np.max(image) <= 1.0
...
def draw_boxes(img, bboxes, color=(0, 0, 1.0), thick=4):
